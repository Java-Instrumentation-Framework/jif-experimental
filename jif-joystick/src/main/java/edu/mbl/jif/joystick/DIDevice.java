...
access to multiple devices. This code is derived from Joerg
Plewe's JXInput package. */
public class DIDevice extends PolledDeviceDI {
    ...
    @param type device type as described in class DeviceCodes.
    @param devidx the index of the device in the list of
    DirectInput devices.
    @param ax this device's axis limit value. This is needed such that
    the driver can scale the axis values into the range [-1.0,
    1.0]. */
    ...
    public void reset() {
        ...
        for (int i = 0; i < diAxis.length; i++) {
            if (diAxis[i] != null) {
                lastAxisValue[i] = axisTransform.valueOf(
                    diAxis[i].getValue());
            }
        }
    ...
    public PolledDeviceDIData read() {
        ...
        return new PolledDeviceDIData(this, changedButtonIndex, buttonState,
    ...
    public String toString() {
        ...
        for (int i = 0; i < diDirectional.length; i++) {
            b.append(
                "\n    " + diDirectional[i].getName() + " = " + diDirectional[i].getDirection());
        }
        return b.toString();
    }
