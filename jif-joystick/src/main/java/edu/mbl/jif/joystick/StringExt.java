...
/*
  ...
  characters are treated as an empty line.
...
public class StringExt {
    ...
    /** Parse the given string containing newline characters into an
	array of strings.
	@param s a string which may contain newline characters.
	@return an array of strings with every single string
	corresponding to a single line of the input string
	argument. */
	...
	StringTokenizer st = new StringTokenizer(s, "\n", true);
	while (st.hasMoreTokens()) {
	    String ss = st.nextToken().replace('\n', ' ');
	    // System.out.println(ss);
	    a.add(ss);
	}
	// System.out.println(a.size());
    ...
    */
    ...
    /** Create a text paragraph by breaking the input text into lines
	according to the given FontMetrics and the requested width.
	@param text the text which should be broken into lines. The
	string should not contain newline characters since these are
	ignored.
	@param fm the FontMetrics which should be used for line
	measuring.
	@param parWidth the intended maximum width for every single line. 
	@return an array of strings which when drawn with the given
	FontMetrics fills the intended width. */ 
    public static ArrayList getTextParagraph(String text, java.awt.FontMetrics fm, int parWidth) {
    ...
    /** Convert a string into an array of strings such that every
	element corresponds to a word of the string. This may be used
	to convert a command line into an args[] array.
	@param cmdLine a string containing a sequence of 'words'
	separated by blank characters.
	@return an array of strings which correspond to the single 'words' of
	the input string or null if the input string is null. */
	...
	@param a an ArrayList which contains a list of strings.
	...
	@param n the string whose index should be found.
	@param a the array of strings where to look for.
	@return the index of n in a or (-1) if n is not contained in
	a. */
    ...
    /** Remove the first occurence of substring t from the string
        s. */
    ...
    /** Replace the first substring t of the string s by the
	...
	@param arr array of strings which is required to be
	sorted.
	...
	for (int i = 1; i < n; i++) {
		...
		case '\'': out[k++] = '\''; break;
		case '\"': out[k++] = '\"'; break;
		default: out[k++] = '\\'; out[k++] = in[i]; break;
		}
		inEscape = false;
	    } else {
		if (in[i] == '\\') {
	...
	try {
	    byte[] c = new byte[1];
	    c[0] = (byte)228; t = replace(t, "&auml;", new String(c, "ISO-8859-1"));
	    c[0] = (byte)246; t = replace(t, "&ouml;", new String(c, "ISO-8859-1"));
	    c[0] = (byte)252; t = replace(t, "&uuml;", new String(c, "ISO-8859-1"));
	    c[0] = (byte)196; t = replace(t, "&Auml;", new String(c, "ISO-8859-1"));
	    c[0] = (byte)214; t = replace(t, "&Ouml;", new String(c, "ISO-8859-1"));
	    c[0] = (byte)220; t = replace(t, "&Uuml;", new String(c, "ISO-8859-1"));
	    c[0] = (byte)223; t = replace(t, "&szlig;", new String(c, "ISO-8859-1"));
    ...
    /** Quote the given string by enclosing it between quote
	characters and replace unprintable characters by escape
	sequences.
	...
	for (int i = 0; i < n; i++) {
	    ...
	    case '\'': out.append("\\\'"); break;
	    case '\"': out.append("\\\""); break;
	    default: out.append(new String(in, i, 1)); break;
	    }
	}
	out.append("\"");
	// System.out.println("Input: " + s);
	...
	@param st the string to be encoded.
    ...
    /** Return an ArrayList of all paths contained int the path list
	...
	if (a != null) {
	    if (a.length > 0)
		s.append(" " + a[0]);
	    for (int i = 1; i < a.length; i++) s.append(", " + a[i]);
	}
	s.append("]");
	return s.toString();
    }
	...
	s.append("[");
	if (a != null) {
	    for (int i = 0; i < a.length; i++) s.append(a[i]? "|": "o");
	}
	s.append("]");
	return s.toString();
    }
	...
	if (a != null) {
	    ...
	    for (int i = 1; i < a.length; i++) s.append(", " + String.valueOf(a[i]));
	}
	s.append("]");
	return s.toString();
    }
	...
	@param s left aligned content of the string.
	@return a string which has a fixed length and contains the
	argument string left aligned. The rest is filled with
	blanks. */
    ...
    /** Create an error message based on the parser's error
        message. 
