...
/** A high resolution clock for measuring response times or other
   short time intervals. This class provides a high resolution clock
   which can be used to measure time intervals with high precision. It
   uses the best available timer.
...
public class HiresClock {
    ...
    /** Delay the current thread by the given number of
	milliseconds. The method uses a combination of non-blocking
	and blocking delay to get optimal results. First the current
	thread is put to sleep using the method sleep() of class
	Thread such that other threads are allowed to use system
	resources during this interval. The final section of the time
	interval given has to use a blocking delay since the sleep()
	method of class Thread offers poor granularity for timing.
    */
	...
	// t2 = getTimeNanos(); System.out.println("HiresClock.delay() actual delay = " + (t2 - t1) + " ns");
    }
 

    /** Delay the current thread by the given number of
	nanoseconds. The method uses a combination of non-blocking
	and blocking delay to get optimal results. First the current
	thread is put to sleep using the method sleep() of class
	Thread such that other threads are allowed to use system
	resources during this interval. The final section of the time
	interval given has to use a blocking delay since the sleep()
	method of class Thread offers pure granularity for timing.
    */
    public static void delayNanos(long ns) {

	long t1, t2; t1 = getTimeNanos();
    ...
    /** Block execution of the currently running program for a given
	number of nanoseconds. This method should be used very
	carefully since it actually blocks execution of the whole
	system.
	@param ns the number of nanoseconds to block.
    */
    ...
    /** Get a Delay object which can be used later. The strategy is to
        get a Delay object, then do something which will take less time
        than the delay itself and then tell the HiresClock to wait
        until the delay is finished. 
	@param d delay duration in milliseconds.
	@return a Delay object which may be sent back later to the
	HiresClock in order to finish the delay period. */
    public static Delay getDelay(int d) {
	return new Delay(getTime(), d);
    }
 

    /** Delay thread execution until the given Delay object's end time has passed. 
	@param d a Delay object which has been created by the
	HiresClock earlier. It contains information about when the
    ...
    /** Get the granularity of the clock. This is the smallest numeric
        difference between successive ticks.
	@return the smallest numeric difference between successive
	clock steps. */
	...
	@param nanos the time to be converted in units of a nanosecond counter.
	...
	@param nanos1 the nanosecond time interval start time.
	@param nanos2 the nanosecond time interval end time.
	...
	@param a an array of data values whose mean
	should be computed.
