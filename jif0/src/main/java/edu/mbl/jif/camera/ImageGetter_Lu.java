...
//   short[][] frameShort = null;
   ...
   public ImageGetter_Lu(int depth, int width, int height,
   ...
   public void setMultiFrame(int _multiFrame, boolean _divide) {
      // default is 1 frame if this isn't called
      div = _divide;
...
//         if (depth == 8) {
//            frameShort = new short[multiFrame][size];
//         } else {
//            frameByte = new byte[multiFrame][size];
//         }
      }
   }
      ...
      if (depth == 8) {
         if (destination instanceof byte[]) {
            if (frmsToAvg == 1) {
               // 8-bit, single --> byte
               if (mirrorImage) {
                  for (int x1 = 0, x2 = (int) width - 1; x1 < width; x1++,
                        x2--) {
                  ...
                  System.arraycopy(buffByte, 0, (byte[]) destination, 0, size);
               }
            } else {
               // 8-bit, multiple --> byte
               if (mirrorImage) {
                  for (int x1 = 0, x2 = (int) width - 1; x1 < width; x1++,
                     ...
                     for (int y = 0; y < height; y++) {
                        off = y * (int) width;
                        if (div) {
                           ((byte[]) destination)[x2 + off] = (byte) ((int) (sumShort[x1
                                 + off]) / frmsToAvg);
                        } else {
                  ...
                  for (int i = 0; i < size; i++) {
                     if (div) {
                        ((byte[]) destination)[i] = (byte) ((int) (sumShort[i])
                        / frmsToAvg);
                     } else {
               ...
               if (mirrorImage) {
                  for (int x1 = 0, x2 = (int) width - 1; x1 < width; x1++,
                     ...
                     for (int y = 0; y < height; y++) {
                        ...
                        ((short[]) destination)[x2 + off] = (short) (buffByte[x1 + off]
                              & 0xff);
                     }
                  }
               } else {
            ...
            if (mirrorImage) {
               for (int x1 = 0, x2 = (int) width - 1; x1 < width; x1++, x2--) {
                  for (int y = 0; y < height; y++) {
                     ...
                     ((short[]) destination)[x2 + off] = (short) (buffShort[x1 + off]
                           /*& 0xffff*/ );
                  }
               }
            } else {
            ...
            if (mirrorImage) {
               for (int x1 = 0, x2 = (int) width - 1; x1 < width; x1++, x2--) {
                  for (int y = 0; y < height; y++) {
                     off = y * (int) width;
                     if (div) {
                        ((short[]) destination)[x2 + off] = (short) (sumInt[x1
                              + off] / frmsToAvg);
                     } else {
