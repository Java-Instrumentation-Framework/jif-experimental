      ...
      implements Cloneable
{
   ...
   public PolStack (String tiffPathFile, boolean raw) {
      ...
      if (raw) {
         ...
         if (dataType == DataBuffer.TYPE_BYTE) {
            if (numImages > 0) {
               imgMagnitude = ((DataBufferByte) (
                     (BufferedImage) imgs.get(0)).getRaster().getDataBuffer()).getData();
            }
            if (numImages > 1) {
               imgAzimuth = ((DataBufferByte) (
                     (BufferedImage) imgs.get(1)).getRaster().getDataBuffer()).getData();
            }
         } else if (dataType == DataBuffer.TYPE_SHORT) {
            if (numImages > 0) {
               imgMagnitude = ((DataBufferShort) (
                     (BufferedImage) imgs.get(0)).getRaster().getDataBuffer()).getData();
            }
            if (numImages > 1) {
               imgAzimuth = ((DataBufferShort) (
                     (BufferedImage) imgs.get(1)).getRaster().getDataBuffer()).getData();
            }
         }
         extSlice = 2;
      ...
      if (dataType == DataBuffer.TYPE_BYTE) {
         if (numImages > extSlice) {
            slice0 = ((DataBufferByte) (
                  (BufferedImage) imgs.get(extSlice)).getRaster().getDataBuffer()).
                     getData();
         }
         if (numImages > extSlice + 1) {
            slice1 = ((DataBufferByte) (
                  (BufferedImage) imgs.get(extSlice + 1)).getRaster().getDataBuffer()).
                     getData();
         }
         if (numImages > extSlice + 2) {
            slice2 = ((DataBufferByte) (
                  (BufferedImage) imgs.get(extSlice + 2)).getRaster().getDataBuffer()).
                     getData();
         }
         if (numImages > extSlice + 3) {
            slice3 = ((DataBufferByte) (
                  (BufferedImage) imgs.get(extSlice + 3)).getRaster().getDataBuffer()).
                     getData();
         }
         if (numImages > extSlice + 4) {
            slice4 = ((DataBufferByte) (
                  (BufferedImage) imgs.get(extSlice + 4)).getRaster().getDataBuffer()).
                     getData();
         }

      } else if (dataType == DataBuffer.TYPE_SHORT) {
         if (numImages > extSlice) {
            slice0 = ((DataBufferShort) (
                  (BufferedImage) imgs.get(extSlice)).getRaster().getDataBuffer()).
                     getData();
         }
         if (numImages > extSlice + 1) {
            slice1 = ((DataBufferShort) (
                  (BufferedImage) imgs.get(extSlice + 1)).getRaster().getDataBuffer()).
                     getData();
         }
         if (numImages > extSlice + 2) {
            slice2 = ((DataBufferShort) (
                  (BufferedImage) imgs.get(extSlice + 2)).getRaster().getDataBuffer()).
                     getData();
         }
         if (numImages > extSlice + 3) {
            slice3 = ((DataBufferShort) (
                  (BufferedImage) imgs.get(extSlice + 3)).getRaster().getDataBuffer()).
                     getData();
         }
         if (numImages > extSlice + 4) {
            slice4 = ((DataBufferShort) (
                  (BufferedImage) imgs.get(extSlice + 4)).getRaster().getDataBuffer()).
                     getData();
         }
      }
