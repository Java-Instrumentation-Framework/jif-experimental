...
final double pi=3.1415926535897932384626433;	// pi
final double twopi=2.0*pi;			// pi times 2
final double two_over_pi= 2.0/pi;		// 2/pi
final double halfpi=pi/2.0;			// pi divided by 2
final double threehalfpi=3.0*pi/2.0;  		// pi times 3/2, used in tan routines
final double four_over_pi=4.0/pi;		// 4/pi, used in tan routines
final double qtrpi=pi/4.0;			// pi/4.0, used in tan routines
final double sixthpi=pi/6.0;			// pi/6.0, used in atan routines
final double tansixthpi=Math.tan(sixthpi);		// tan(pi/6), used in atan routines
final double twelfthpi=pi/12.0;			// pi/12.0, used in atan routines
final double tantwelfthpi=Math.tan(twelfthpi);	// tan(pi/12), used in atan routines
...
{
final float c1= 0.99940307f;
final float c2=-0.49558072f;
final float c3= 0.03679168f;
...
float cos_32(float x){
	...
	x=(float)(x % twopi);				// Get rid of values > 2* pi
	if(x<0)x=-x;					// cos(-x) = cos(x)
	quad=(int)(x * two_over_pi);			// Get quadrant # (0 to 3) we're in
	switch (quad){
	case 0: return  cos_32s(x);
	case 1: return -cos_32s((float)pi-x);
	case 2: return -cos_32s((float)(x-pi));
	case 3: return  cos_32s((float)twopi-x);
	}
}
//
...
{
final float c1= 0.9999932946;
final float c2=-0.4999124376;
final float c3= 0.0414877472;
final float c4=-0.0012712095;
...
final double c5= 0.000023233;  	// Note: this is a better coefficient than Hart's
...
{
final double c1= 0.99999999999925182;
final double c2=-0.49999999997024012;
final double c3= 0.041666666473384543;
final double c4=-0.001388888418000423;
final double c5= 0.0000248010406484558;
final double c6=-0.0000002752469638432;
final double c7= 0.0000000019907856854;
...
{
final float c1=-3.6112171;
final float c2=-4.6133253;
...
{
final float c1=-3.16783027;
final float c2= 0.134516124;
final float c3=-4.033321984;
...
{
final double c1=-34287.4662577359568109624;
final double c2=  2566.7175462315050423295;
final double c3=-   26.5366371951731325438;
final double c4=-43656.1579281292375769579;
final double c5= 12244.4839556747426927793;
final double c6=-  336.611376245464339493;
...
{
final double c1= 48.70107004404898384;
final double c2= 49.5326263772254345;
final double c3=  9.40604244231624;
final double c4= 48.70107004404996166;
final double c5= 65.7663163908956299;
final double c6= 21.587934067020262;
...
// Print a header line identifying the meaning of each column of data
	printf(results,"x, Cosine, Sine, Tangent, aTan");
	printf(results,",cos_32  Error, sin_32 Error");
	printf(results,",cos_52  Error, sin_52 Error");
	printf(results,",cos_73  Error, sin_73 Error");
	printf(results,",cos_121 Error, sin_121 Error");
	printf(results,",tan_32  pc Error");
	printf(results,",tan_56  pc Error");
	printf(results,",tan_82  pc Error");
	printf(results,",tan_14  pc Error");
	printf(results,",atan_66 Error");
	printf(results,",atan_137 Error");
	printf(results,"\n");
...
// Run the approximations over the entire range
	...
	for(i=0;i<361;i+=1){	
		...
		printf(results,"%d,%1.14f,%1.14f, %1.14f",i, cos(b), sin(b), tan(b), atan(tan(b)));
		printf(results,",%1.14f,%1.14f",cos(b)- cos_32(b), sin(b)-sin_32(b));
		printf(results,",%1.14f,%1.14f",cos(b)- cos_52(b), sin(b)-sin_52(b));
		printf(results,",%1.14f,%1.14f",cos(b)- cos_73(b), sin(b)-sin_73(b));
		printf(results,",%1.14f,%1.14f",cos(b)- cos_121(b),sin(b)-sin_121(b));
		printf(results,",%1.14f",100.0*(tan(b)-tan_32(b))/tan_32(b));
		printf(results,",%1.14f",100.0*(tan(b)-tan_56(b))/tan_56(b));
		printf(results,",%1.14f",100.0*(tan(b)-tan_82(b))/tan_82(b));
		printf(results,",%1.14f",100.0*(tan(b)-tan_14(b))/tan_14(b));
		printf(results,",%1.14f",atan(tan(b))-atan_66(tan(b)));
		printf(results,",%1.14f",atan(tan(b))-atan_137(tan(b)));
		printf(results,"\n");
	}    
