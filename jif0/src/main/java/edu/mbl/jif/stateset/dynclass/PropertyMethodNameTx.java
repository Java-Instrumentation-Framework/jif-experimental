...
class PropertyMethodNameTx {
    ...
    public static String encodeProperty(String str) {
        ...
        // check whether property can be upper-cased, or full encoding
        // is required
        if (str.length() == 0 || 
        ...
        String encoded = Character.toUpperCase(str.charAt(0)) +
            str.substring(1);
        String decap = Introspector.decapitalize(encoded);
    ...
    private static String decodeArbitrary(String str, int startIndex) {
        ...
        while (i < len) {
            ...
            if (ch != '$') {
                ...
                // read everything between current char and next '$'
                // as decimal value
                int val = 0;
                try {
                    ch = str.charAt(i++);
