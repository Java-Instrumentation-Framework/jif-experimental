package edu.mbl.jif.varilc.mmgr;
...
///////////////////////////////////////////////////////////////////////////
//  VariLCCommMM - interfaces to microManager...
 ...
 */
public class VariLCCommMM implements VLCComm {
...
//      pStackIcon[1] = loadIcon("pstackiconazim.gif");
	...
	static ImageIcon loadIcon(String file) {
		return JifUtils.loadImageIcon(file, VariLCCommMM.class);
	}

	SerialPortConnection io; // <<<<<<<<<<<<<<<<<  REMOVE THIS !!!!
	...
	private double DEFAULT_WAVELENGTH = 540;

	public VariLCCommMM(CMMCore mmc) {
		this.mmc = mmc;
	...
	//
    /*
	 * public int initializeController() { // If (Include VariLC_RT in Initialization)& String resp; boolean responsive
	 * = checkIfResponsive(); if (!responsive) { // DialogBoxes.boxError("VariLC_RT Initialization Error", // "Port
	 * opened, but VariLC_RT does not respond.\n" // + // "(Quit and re-start PSjCheck after checking VariLC_RT and
	 * connections.)"); // PSjUtils.event("VariLC_RT initialization failed"); System.err.println("VariLC_RT
	 * initialization failed"); isFunctional = false; return 0; } reset(); setCommandFormat(1); // brief // if
	 * (!checkSerialNumber(getVersion())) { // DialogBoxes.boxError("VariLC_RT Serial Number Error", // "VariLC_RT
	 * serial number incorrect."); // PSjUtils.event("VariLC_RT serial number incorrect"); // isFunctional = false; //
	 * return 0; // } setUnits(0); // nanometers float wavelength = PrefsRT.usr.getFloat("wavelength", 546);
	 * setWavelength(wavelength); resp = sendCommandAndWait("I?", 500); // initialize isCalibrated = false; // load
	 * setting from saved properties // loadSettings(); isFunctional = true; //PSjUtils.event("VariLC_RT initialized");
	 * System.out.println("VariLC_RT initialized"); return 0; }
	 */
