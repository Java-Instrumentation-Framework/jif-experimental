...
public class Calc_5Fr implements PlugInFilter {
	ImagePlus imp1, imp2;
...
//	float intCeiling = 255f; // typically 255 or 4095, if set to 1 then ret pixel values are retardance in nm
	...
	public void Calc_5Fr(ImagePlus imp1, ImagePlus imp2) {
		if (imp1.getType()!=imp2.getType()) {error(); return;}
...
//		if (bgStackTitle != "NoBg") ImageStack stack2 = imp2.getStack();
		...
		float a; float b; float den; 
		float aSmp; float bSmp; float denSmp; 
		float aBg; float bBg; float denBg; 
		float retF; float azimF;
...
//terms a and b
		for (int j=0;j<dimension;j++) {
			denSmp = (pixF[1][j]+pixF[2][j]+pixF[3][j]+pixF[4][j]-4*pixF[0][j])/2; denBg=denSmp;
			a = (pixF[4][j]-pixF[1][j]); aSmp = a; aBg = a;
			b = (pixF[2][j]-pixF[3][j]); bSmp = b; bBg = b;
			if (bgStackTitle!="NoBg") {
...
// Special case of sample retardance half wave, denSmp = 0
			if (denSmp==0) {
				...
				azimF = (float) (a == 0 & b == 0 ? 0 :
					(azimRef+90+90*Math.atan2(a,b)/Math.PI)%180);
			} else {
...
// Orientation
				if ((bgStackTitle=="NoBg")||((bgStackTitle!="NoBg")&&(Math.abs(denSmp)<1))) {a=aSmp; b=bSmp;}
				azimF = (float) (a == 0 & b == 0 ? 0 :
					(azimRef+90+90*Math.atan2(a,b)/Math.PI)%180);
			}
			if (bitDepth=="8-bit") 
