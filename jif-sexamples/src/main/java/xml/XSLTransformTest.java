 ...
 *   java XSLTransformTest makeprop.xsl
 */;
public class XSLTransformTest {
    public static void main(String[] args) throws Exception {
        String testDir = "./testData/xml/";
        String filename;
        if (args.length > 0) {
            filename = args[0];
        } else {
            filename = testDir + "makeprop.xsl";
        }
        File styleSheet = new File(filename);
        StreamSource styleSource = new StreamSource(styleSheet);
        Transformer t = TransformerFactory.newInstance().newTransformer(styleSource);
        t.transform(new SAXSource(new EmployeeReader(),
                new InputSource(new FileInputStream(testDir+"employee.dat"))), new StreamResult(System.out));
    }
}
