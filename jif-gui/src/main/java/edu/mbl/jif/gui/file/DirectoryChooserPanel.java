 ...
 * @version $Id: DirectoryChooserPanel.java,v 1.2 2002/03/23 02:53:28 tonyj Exp $
 */
...
public static final int FILES_AND_DIRECTORIES =
  ...
  /**
   * Create a DirectoryChooserPanel with the default FileSystemView
   */
   ...
   */
  public DirectoryChooserPanel(File currentDirectory) {
    this();
   ...
   * @param currentDirectory The directory to which the tree is initially set
   */
  public DirectoryChooserPanel(String currentDirectory) {
    this();
  ...
  /**
   * Create a DirectoryChooserPanel
   * @param currentDirectory The directory to which the tree is initially set
   * @param view The FileSystemView to use
   */
  public DirectoryChooserPanel(File currentDirectory, FileSystemView view) {
    this(view);
    setCurrentDirectory(currentDirectory);
  }
   ...
   * @param view The FileSystemView to use
   */
  public DirectoryChooserPanel(String currentDirectory, FileSystemView view) {
    this(view);
    setCurrentDirectory(new File(currentDirectory));
  }
   ...
   * @param view The FileSystemView to use
   */
  public DirectoryChooserPanel(FileSystemView view) {
    setup(view);
...
//        TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION);
   ...
   * @param mode either DIRECTORIES_ONLY or FILES_AND_DIRECTORIES (the default)
  ...
  public void setMultiSelectionEnabled(boolean enable) {
    ...
    tree.getSelectionModel().setSelectionMode(enable ?
        TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION :
        TreeSelectionModel.SINGLE_TREE_SELECTION);
  }
  ...
  /**
   * Popup up a modal dialog containing the DirectoryChooserPanel
  ...
  private class TreeListener
    ...
    public void valueChanged(TreeSelectionEvent e) {
      System.out.println("Tree valueChanged " + e.getPaths());
      System.out.println("TreeSelected: " + tree.getSelectionCount());
  ...
  private class FileRenderer extends DefaultTreeCellRenderer {
    public Component getTreeCellRendererComponent(JTree tree, Object node,
        ...
        boolean hasFocus) {
      Component comp = super.getTreeCellRendererComponent(tree, node, selected,
  ...
  private class FileTreeModel
    ...
    private File[] children(Object node) {
      ...
      try {
        ...
        File[] children = isRoot ? view.getRoots() : view.getFiles(dir,
            foo.isFileHidingEnabled());
        if (!isRoot) {
  ...
  public static void main(String argv[]) {
    ...
    JButton b = new JButton("Show Browser") {
      public void fireActionPerformed(ActionEvent e) {
        DirectoryChooserPanel dlg = new DirectoryChooserPanel("C:/MicroManagerData"); //tf.getText());
        dlg.showDialog(this);
        //dlg.showInternalDialog();
        if (dlg.getSelectedFile() != null){
            String pathNormalized = dlg.getSelectedFile().toString().replace('\\', '/');
          tf.setText(pathNormalized);
        }
      }
    };
