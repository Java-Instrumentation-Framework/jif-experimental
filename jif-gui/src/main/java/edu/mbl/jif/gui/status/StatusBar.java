...
public class StatusBar extends JPanel implements StatusInterface {
    ...
    private LedStatus sendLed;
    private LedStatus receiveLed;
    ...
    public void countingProgressbarIncrement() {
        count++;
        if (showInPercent) {
            int percent = 100*count/maxCount;
    ...
    private LedStatus getSendLed() {
        if (sendLed == null) {
            sendLed = new LedStatus(LedStatus.LedShape.RECTANGLE);
            sendLed.setPreferredSize(new Dimension(15, 10));
        }
        return sendLed;
    }
    
    private LedStatus getReceiveLed() {
        if (receiveLed == null) {
            receiveLed = new LedStatus(LedStatus.LedShape.RECTANGLE);
