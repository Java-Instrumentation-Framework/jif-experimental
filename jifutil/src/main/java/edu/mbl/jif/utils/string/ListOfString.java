...
/**
 * <p>Description: Encapsulates a list whose items are String objects. This class
 *   extends the ArrayList class. Some interesting features are:</p>
 * <ol>
 ...
 * <li>Items can be in the form: <code>name=value</code>. With methods to access
 * by name.
 ...
 * @version 1.0
 */
public class ListOfString extends java.util.ArrayList {

  /** Case sensetivity for searches. Default=true */
...
public int indexOf(String aItem) {
  ...
  for(int i=0,n=size(); i<n; i++) {
    if ( getItem(i).equalsIgnoreCase(aItem)) return i;
  } // for
  return -1;
} // indexOf
...
public String getValueAtIndex(int aIndex) {
  ...
  int equalIndex = item.indexOf(valueEquator);
  if (equalIndex<0) {return item; }
  else return item.substring(equalIndex + valueEquator.length());
} // getValueAtIndex
...
* Will close the reader when done.
* @param aStream The buffered reader to read from.
...
public static void main(String[] args) {
  ...
  ListOfString list2= new ListOfString(new String[] {"a", "1", "2"} );

  System.out.println(list.intersect(list2).toString() );
}
