 ...
 *
 * Modified by gharris@mbl.edu 2007
 */
public class SerialParameters {
    ...
    portName       = ""
    baudRate       = "9600"
    flowControlIn  = FLOWCONTROL_NONE
    flowControlOut = FLOWCONTROL_NONE
    parity         = PARITY_NONE
    databits       = DATABITS_8
    stopbits       = STOPBITS_1
     */
    /**
     * Default constructer.
     * Sets parameters to no port, 9600 baud, no flow control, 8 data bits, 1 stop bit, no parity.
     */
    public SerialParameters() {
        props = new SerialProperties();
        loadParamsFromProps();
    }
     ...
     */
    private void loadParamsFromProps() {
    ...
    private void putParamsToProps() {
        props.put("portName", getPortName());
        props.put("baudRate", getBaudRateString());
        props.put("flowControlIn", String.valueOf(getFlowControlIn()));
        props.put("flowControlOut", String.valueOf(getFlowControlOut()));
        props.put("parity", String.valueOf(getParity()));
        props.put("databits", String.valueOf(getDatabits()));
        props.put("stopbits", String.valueOf(getStopbitsString()));
    }
     ...
     */
    static String flowToString(int flowControl) {
        switch (flowControl) {
    ...
    public String getAllString() {
        return getBaudRateString() + "," + this.getDatabitsString() + "," +
                this.getStopbitsString() + "," + this.getParityString() + "," +
                this.getFlowControlInString() + "," + this.getFlowControlOutString();
    }
    // <editor-fold defaultstate="collapsed" desc="<<< SerialProperties >>>">

    /*
    Save / retrieve parameters from properties file in current working directory
    named portCOMn.properties, it looks like:
    #Serial properties for COM5
    #Fri Oct 10 12:56:37 EDT 2008
    flowControlIn=0
    flowControlOut=0
    stopbits=1
    databits=8
    ...
    baudRate=9600
     */
    private class SerialProperties
            extends Properties {
        ...
        private void init() {
            ...
            defaults.put("portName", "");
            defaults.put("baudRate", "9600");
            defaults.put("flowControlIn", String.valueOf(SerialPort.FLOWCONTROL_NONE));
            defaults.put("flowControlOut", String.valueOf(SerialPort.FLOWCONTROL_NONE));
            defaults.put("parity", String.valueOf(SerialPort.PARITY_NONE));
            defaults.put("databits", String.valueOf(SerialPort.DATABITS_8));
            defaults.put("stopbits", String.valueOf(SerialPort.STOPBITS_1));
        }
        ...
        public void retreive(String portName) throws IOException {
            if (!portName.equalsIgnoreCase("")) { // Load from file
                try {
                    FileInputStream fis = new FileInputStream(getcwd() + "/port" + portName + ".properties");
                    load(fis);
                    fis.close();
                } catch (FileNotFoundException e1) {
                    System.out.println("No parameters setting file found for " + portName + "; using default settings.");
        ...
        public void store(String portName) throws IOException {
            FileOutputStream fileOut = null;
            fileOut = new FileOutputStream(getcwd() + "/port" + portName + ".properties");
            super.store(fileOut, "Serial properties for " + portName);
            fileOut.close();
        }
    }    // </editor-fold>
}
