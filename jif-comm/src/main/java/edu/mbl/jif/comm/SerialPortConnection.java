...
//import edu.mbl.jif.utils.StringUtil;

/**
 * A class to handle communications with a serial connection based on Vern Raben's Telescope code
 */// Defaults:
...
public class SerialPortConnection
	...
	/**
	 * Create a SerialConnection using DEFAULT settings of 8 bits, 9600 baud, no parity, no flow control, and 1 stop bit
	 * on port named COM1 . @todo ???????
	 */
...
//    }
	...
	public void openConnection() throws IOException {
		...
		try {  // Open the port
			serialPort = (SerialPort) portId.open(portOwner, 30000);
...
//        }
	 ...
	 * to origional settings and throws exception.
	 */
	...
	/**
	 * -------------------------------------------------------------------------- Set name of port to the one specified or
	 * else the first one avail. as default.
	 *
	...
	/**
	 * Set flow control Formed by "or-ing" both "in" and "out" flow control settings. The default is
...
//            }
			...
			/*
			 * case SerialPortEvent.DATA_AVAILABLE: byte[] readBuffer = new byte[1024];
			 *
			 * // System.out.println("DATA_AVAILABLE"); try { while (is.available() > 0) { // available() does not block int
			 * numBytes = is.read(readBuffer); if (monitor != null) { monitor.received(readBuffer, 0, numBytes); }
			 *
			 * try { //String ascii = new String(data, 0, len, "ANSI_X3.4-1968"); String ascii = new String(readBuffer, 0,
			 * numBytes); //,"ASCII"); messageQueue.addMessage(ascii); // System.out.println("recvd: " + ascii); } catch
			 * (Exception eio) { eio.printStackTrace(); }
			 *
			 * // System.out.println("bytes: " + numBytes + " - " + // new String(readBuffer)); } //System.out.print(new
			 * String(readBuffer)); } catch (IOException e) { System.out.println("\n" + e.toString() + "\n"); } break;
			 */
		}
	}
	// </editor-fold>
	...
	/**
	 * Handle ownershipChange event. When PORT_OWNERSHIP_REQUESTED event is received, close the connection.
	 *
	...
	/**
	 * -------------------------------------------------------------------------- Send an array of chars to the serial
	 * connection
	 *
	 * @param chars Array of chars
	 * @exception IOException may occur No termination char appended
	 */
	 ...
	 * @throws IOException may occur Termination char appended
	 */
	...
	/**
	 * -------------------------------------------------------------------------- Convert array of bytes to space
	 * separated hex string for debug display
	 *
	 * @param chars Array of chars
	...
	public static void main(String[] args) {
		...
		System.out.println("Ports: " + asString(ps, "\n"));
